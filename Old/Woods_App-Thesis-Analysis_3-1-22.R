...
server <- function(input, output) {
    ...
    # Reading in Data
    # Observe event means, when the "load_file" button is clicked do these things
    ...
    output$fsa <- renderPrint({
        req(user$phase_3_max_force)
        cat("Stretch-Activated Tension: ", round(user$phase_3_max_force, 6)*1000, " mN")
    })
}
