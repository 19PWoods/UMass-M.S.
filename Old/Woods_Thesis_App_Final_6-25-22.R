    ...
    sidebarPanel(
      ...
      conditionalPanel(
        ...
        downloadButton("download_amp", 
                       "Download Amplitude"),
        downloadButton("download_p3_parameters", 
                       "Download Phase 3 Parameters")),
   ...
   mainPanel(
    ...
    tabsetPanel(id = "tabselected",
      tabPanel("Amplitude", value = 1,
               plotOutput("phase_3"),
               tableOutput("phase_3_parameters")),
      
      tabPanel("Rates", value = 2,
               plotOutput("fit"),
               plotOutput("fit_seperated"),
               tableOutput("rates"))
    )
   ) 
)
...
server <- function(input, output){
  ...
  observeEvent(input$set_phase_3, {
    ...
    if(!is.null(input$interactive_plot_date_window)){
      ...
      user$phase_3_parameters <- list(user$phase_3_boundaries[[1]],
  ...
  output$phase_3_parameters <- renderTable({
    req(user$phase_3_parameters)
    user$phase_3_parameters
  }) 
  ...
  output$downloadTable <- downloadHandler(
    ...
    content = function(file) {
      write.csv(user$phase_3_parameters, file = file)
    }
  )
  ...
  output$fit_separated <- renderPlot({
  ...
  output$downloadPlot <- downloadHandler(
    filename = function() {
      paste("Woods_Fiberx_Conditionx_Rates_ggplot", '.png', sep = '')
    },
    content = function(file) {
